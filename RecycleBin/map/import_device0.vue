...
<template>
  <ul class="amap-page-container">
    ...
    <el-collapse v-model="activeNames">
      <el-collapse-item title="所需填入的信息:点击此处可以收放填信息的页面" name="1">
        <el-row :gutter="8">
          <el-col
            :xs="{ span: 32 }"
            :sm="{ span: 32 }"
...
import { creatCJDevice } from "@/api/cj";
...
export default {
  ...
  methods: {
    ...
    judge_type() {
      if (this.query.type.trim().length === 0) {
    ...
    judge_serial() {
      if (this.isLetter(this.query.serial) || (this.query.serial.trim().length === 0)) {
    ...
    judge_adress() {
      if (this.query.address.trim().length === 0) {
